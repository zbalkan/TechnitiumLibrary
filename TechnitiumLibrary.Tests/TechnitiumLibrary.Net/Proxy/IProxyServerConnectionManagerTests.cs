...
namespace TechnitiumLibrary.Tests.TechnitiumLibrary.Net.Proxy
{
    ...
    public sealed class IProxyServerConnectionManagerTests
    {
        ...
        [TestMethod]
        public async Task ConnectAsync_MustNotLeakSocket_OnFailure()
        {
            IProxyServerConnectionManager manager = new ContractTestConnectionManager();
            ...
            try
            {
                ...
                Assert.Fail("Connection to unreachable endpoint must fail.");
            }
            ...
            // No direct observable handle leaks are possible,
            // but reaching here without hangs or resource exhaustion
            // validates failure hygiene contract.
            Assert.IsTrue(true,
                "Failure must complete cleanly without leaking sockets or hanging.");
        }
